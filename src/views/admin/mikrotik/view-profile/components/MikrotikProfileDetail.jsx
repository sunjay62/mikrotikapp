...
const MikrotikProfileDetail = () => {
  const [routerNames, setRouterNames] = useState([]);
  const mikrotikIdList = useMikrotikStore((state) => state.mikrotikIdList);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const mikrotikIds = mikrotikIdList.map((mikrotik) => mikrotik.id);
  ...
  useEffect(() => {
    setRouterNames(mikrotikIdList.map((mikrotik) => mikrotik.name));
  }, [mikrotikIdList]);
...
export default MikrotikProfileDetail;
