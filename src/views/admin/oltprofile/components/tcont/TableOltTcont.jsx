...
export function TableOltTcont() {
  const [open, setOpen] = useState(false);
  const [openEdit, setOpenEdit] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const { data, refetch } = useData();
  const [filteredUsers, setFilteredUsers] = useState([]);
  const [selectedTcont, setSelectedTcont] = useState(null);
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      title: "Delete Tcont",
      text: "Are you sure to delete this Tcont?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await toast.promise(
            axiosInstance.delete(`${BASE_URL_OLT}/speedprofile/tcont`, {
              ...config,
              data,
            }),
            {
        ...
        >
          <div className="mb-8 items-center justify-between gap-8">
            ...
            <p color="gray" className="mt-1 font-normal">
              See information about all Tcont
            </p>
          </div>
            ...
            {!roleHidden && (
              <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                <Button
                  ...
                  onClick={handleOpen}
                >
                  <BsPlus strokeWidth={2} className="h-4 w-4" /> Add Tcont
                </Button>
              </div>
            )}
          </div>
