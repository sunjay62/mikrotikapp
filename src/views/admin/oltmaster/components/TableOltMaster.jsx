...
import Compatible from "./useCompatible";
import {
  Card,
  CardHeader,
  Button,
...
const TABLE_HEAD = ["No", "Merk", "Model", ""];

export function TableOltMaster() {
  ...
  const handleOpenSoftware = (items) => {
    setSelectedSoftware(items);
  ...
  const handleDelete = async (item) => {
    ...
    Swal.fire({
      title: "Delete OLT Master",
      text: `Are you sure to delete this ${merk} OLT Master?`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          await toast.promise(
            axiosInstance.delete(`${BASE_URL_OLT}/oltmaster/merk`, {
              ...config,
              data,
            }),
            {
              pending: "Deleting ...",
              success: `Deleted  ${merk} Successfully!`,
            }
          );
  ...
  return (
    <>
        ...
        >
          <div className="mb-8 items-center justify-between gap-8">
            ...
            <p color="gray" className="mt-1 font-normal">
              See information about all OLT Master
            </p>
          </div>
            ...
            {!roleHidden && (
              <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                <Button
                  ...
                  onClick={handleOpen}
                >
                  <BsPlus strokeWidth={2} className="h-4 w-4" /> Add OLT Master
                </Button>
              </div>
            )}
          </div>
                      ...
                      <td className={`${classes}`}>
                        <div className="flex justify-end">
                          {!roleHidden && (
                            <>
                              <Tooltip
                                content="Compatible Software"
                                className="bg-gray-700"
                              >
                                <IconButton
                                  variant="text"
                                  className="ml-2 border bg-blue-50 hover:bg-blue-100 dark:bg-green-800 dark:hover:bg-blue-200"
                                  onClick={() => handleOpenSoftware(items)}
                                >
                                  <MagnifyingGlassPlusIcon className="h-5 w-5 text-green-400" />
                                </IconButton>
                              </Tooltip>
      ...
      <Compatible
        ...
        handleOpenSoftware={handleOpenSoftware}
        items={selectedSoftware}
      />
    </>
  );
}

export default TableOltMaster;
